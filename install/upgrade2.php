        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->tableExists($table)) {
                    ...
                    $sql .= ' `prefix` varchar(20) COLLATE utf8_unicode_ci NOT NULL DEFAULT "",';
                    $sql .= ' `auto_increment` int(11) NOT NULL,';
                    $sql .= ' `last_update` date NOT NULL';
                    $sql .= ') ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci ADD PRIMARY KEY (`type`,`prefix`)';
                    $db->query($sql);
                } else {
                    $sql = 'ALTER TABLE `'.$table.'` CHANGE `prefix` `prefix` VARCHAR(20) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT ""';
                    $db->query($sql);
                }
                $content[] = '<li class="correct">ปรับปรุงตาราง `'.$table.'` สำเร็จ</li>';
                // ตาราง enroll_plan
